...
let dx = 0;
let dxVelocity = 0.5; // world units per second
const CUBE_HALF = 0.05; // since our cube vertices are defined in the range [-0.5, 0.5], we want to limit translation to keep the cube within view
const DX_LIMIT = 1 - CUBE_HALF;

let dy = 0;
let dyVelocity = 0.5; // world units per second
const DY_LIMIT = 1 - CUBE_HALF;

// translate a point in 3D space by adding dx to its x coordinate and dz to its z coordinate
...
function transformVertex(v, angle, dx, dy, dz) {
    return translate(
        rotateXZ(
            rotateXY(
                rotateYZ(v, angle), 
...
function frame() {
    ...
    dx += dxVelocity * dt;
    dy += dyVelocity * dt;
    //dz += 0.5 * dt; // move forward at a constant speed of 0.5 world units per second
    ...
    vs.forEach((v, i) => {
        ...
        drawCoordinates();
        const v3d = transformVertex(v, angle, dx, dy, dz);
        // Only render if vertex is in front of camera
        if (isInFrontOfCamera(v3d)) {
            point(screen(project(v3d)), i, dt);
        }
    });
    ...
    for (const f of fs) {
        for (let i = 0; i < f.length; i++) {
            ...
            const v1_3d = transformVertex(v1, angle, dx, dy, dz);
            const v2_3d = transformVertex(v2, angle, dx, dy, dz);
